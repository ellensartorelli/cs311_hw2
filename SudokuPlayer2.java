...
public class SudokuPlayer implements Runnable, ActionListener {
    ...
    private final  void setupGlobalDomains() {
        for (int x = 0; x < 9; x++) {
            for (int y = 0; y < 9; y++) {
                //check that order isn't flipped
                if(vals[x][y] != 0){
    ...
    private final boolean backtrack(int cellx, int celly, ArrayList<Cell> cellCopy) {
        ...
        if(cellx == 8 && celly == 8){
        	for(int i = 0; i < 9; i++){
        		for(int j = 0; j < 9; j++){
        			vals[i][j] = cells[i][j].domain.get(0);
        		}
        	}
            return true;
        }
        ...
        if(vals[cellx][celly] != 0){
        	Cell currentCell = cells[cellx][celly];
        	for(Integer value : currentCell.domain){
        		currentCell.setDomain(value);
        		 //next
            	if(cellx == 8){
            		backtrack(0, celly++, cellCopy);
            	}else{
            		backtrack(cellx++, celly, cellCopy);
            	}
        	}
    ...
    private final boolean AC3(ArrayList<Cell> cellCopy) {
    	...
    	while(!localQueue.isEmpty()){
    		...
    		if(Revise(current)){
    			Cell x = cells[current.ix][current.iy];
    			if(x.isEmpty()){
    				return false;
    			}else{
    ...
    private final boolean Revise(Arc current){
        ops += 1;
        
        Cell x = cells[current.ix][current.iy];
        Cell y = cells[current.jx][current.jy];
    ...
    class Cell {
        ...
        public void printNeighbors() {
            for(int i = 0; i <20; i++){
                System.out.println("("+neighborsX[i] + ", " + neighborsY[i]+")");
            }
        ...
        public boolean isNeighbor(int x, int y){
            for(int i = 0; i <20; i++){
                if (neighborsX[i] == x && neighborsY[i] == y){
                    return true;
                }
            }
            return false;
