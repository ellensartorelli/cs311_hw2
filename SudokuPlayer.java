...
//Joy Wood
//Ellen Sartorelli
//All groups members were present while working on this project.
...
public class SudokuPlayer implements Runnable, ActionListener {
	...
	private final void allDiff() {
		//calls allArcs and allNeighbors for every tile to set up constraints
		for (int i = 0; i < 9; i++) {
			for (int j = 0; j < 9; j++) {
				allArcs(i, j);
				allNeighbors(i, j);
	...
	private final void setupGlobalDomains() {
		//create global domain queue by iterating through board 
		for (int x = 0; x < 9; x++) {
			for (int y = 0; y < 9; y++) {
				// if the value is pre-coded
				if (vals[x][y] != 0) {
					//then set the domain array of the cell object to the pre-coded value
					cells[x][y].setDomain(vals[x][y]);
				}
				//else
				// the domain array remains the default 1 - 9 that was initialized when the cell object was created
			}
		}
	...
	private final void allNeighbors(int x, int y) {
		...
		// find the neighbors in the same row
		...
		// find the neighbors in the same column
		...
		// set up offsets to iterate through boxes
		int offsetX = (x / 3) * 3;
		int offsetY = (y / 3) * 3;
		
		// find the neighbors in the same box
		...
		// create a new cell for each value on the board
		// XValues and YValues are the x and y values of the neighboring cells
		// x and y are the cell's x-y position
	...
	private final boolean custom(Cell[][] cellsPass) {
		recursions += 1;
		...
		// set initial values 
		int minDomain = 10; //there will always be a domain lower than this "minDomain"
		int minDomainX = 0;
		int minDomainY = 0;

		// set initial values for the next cell to visit
		// will be modified to visit the next cell with the lowest domain
		int nextCellX = 0;
		int nextCellY = 0;
		int seenCount = 0;
		...
		for (int i = 0; i < 9; i++) {
			for (int j = 0; j < 9; j++) {
				// if this cell has not been seen
				if (!cellsPass[i][j].isSeen()) {
					// if this cell has only one item left in the domain
		...
		// if we have seen all of the tiles
		if (seenCount == 81){
			// then fill our final vals array with the final values
			for (int i = 0; i < 9; i++) {
				for (int j = 0; j < 9; j++) {
					vals[i][j] = cellsPass[i][j].domain.get(0);
				}
		...
		// iterate through the domain of our chosen cell
	...
	private final boolean AC3(Cell[][] cellsCopy) {
		...
		while (!localQueue.isEmpty()) {
			...
			if (Revise(current, cellsCopy)) {
				...
				// if the domain of the cell is empty, return false
				if (x.isEmpty()) {
					return false;
				} else {
					// else enqueue's all of its neighbors
	...
	private final boolean Revise(Arc current, Cell[][] cellsCopy) {
		...
		for (Integer valueX : x.domain) {
			// if there is only one value in y's domain and it === valueX, then delete value from x's domain
